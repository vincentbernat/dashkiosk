...
module.exports = {
  ...
  db: {
    ...
    options: {
      dialect: 'sqlite',
      storage: process.env.npm_package_config_db || path.join(rootPath, 'dashkiosk.sqlite')
    }
  },
  port: process.env.npm_package_config_port || process.env.PORT || 9400,
  env:  env,
  logLevel: (env === 'development') ? 'debug' : 'info',
  secretKey: process.env.npm_package_config_secret || 'changeThisLameSecretPassphrase!!'
};
