...
(function() {
  ...
  Group.prototype.start = function() {
    ...
    function next() {
      ...
      self.getDashboards()
        .then(function(dashboards) {
          ...
          // Let's try to find our current dashboard. It may have moved.
          if (storage.currentDashboard) {
            var guess = _.findIndex(dashboards, function(d) {
              return d.toJSON().id === storage.currentDashboard.toJSON().id;
            });
            storage.currentDashboard = (guess !== -1) && dashboards[guess + 1]  || dashboards[0];
          } else {
            storage.currentDashboard = dashboards[0];
          }
          display();
        });
    }
    ...
    var handle = {
      ...
      // Broadcasts temporarily replace every dashboard in every group.
      broadcast: function(data) {
        var dashboard = new models.Dashboard(data.url, {'timeout': data.timeout || 10});

        if (storage.timer !== null) {
          clearTimeout(storage.timer);
        }
        if (dashboard.toJSON().timeout && dashboard.toJSON().timeout > 0) {
          storage.timer = setTimeout(next, dashboard.toJSON().timeout * 1000);
        }
    ...
    _.extend(storage.subscriptions, [
      ...
      bus.subscribe('broadcast', handle.broadcast)
    ]);
